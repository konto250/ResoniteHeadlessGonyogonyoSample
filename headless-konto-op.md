...
null以外の値を指定すると、ダッシュメニューの「Session」タブにある「Save As...」オプションと同様の動作となり、保存のたびに新しいレコードが作成されます。

| 値 | 保存先 | 備考 |
|---|---|---|
| `"LocalMachine"` | ヘッドレスPCローカル | 最も安定。クラウドへの反映は別途必要 |
| `"CloudUser"` | ヘッドレスクライアントのloginCredentialのユーザーのクラウド | 起動時にloginCredentialのユーザーのクラウドに保存し、saveAsOwnerをnull、loadWorldURLを保存した新たなレコードURLに変更する（ヘッドレスPCローカルにも保存） |
| `null`（デフォルト） | 読み込み元レコードに上書き | 読み込み元レコードへの書き込み権限がある場合に保存される（ヘッドレスローカルPCにも保存）。読み込み元レコードへの書き込み権限がない場合には保存されない（ヘッドレスPCローカルにも保存されない） |
